...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  user_id TEXT NOT NULL, -- Can store either users.id or workers.id as text
...
-- Create assignment history table with correct UUID references
CREATE TABLE IF NOT EXISTS assignment_history (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  request_id UUID REFERENCES requests(id) NOT NULL,
  worker_id UUID REFERENCES workers(id), -- Fixed: UUID not INTEGER
  assigned_by INTEGER REFERENCES users(id), -- Keep as INTEGER for users table
